...
export function versioned<T>(options: IVersionsOptions<T>): (new (...args) => T) & IVersioned {
    ...
    const handler = {
        construct(target, args) {
            try {
                ...
                console.error(`Something went wrong for ${global['version']}, ${target.name}  versions: ${options.versions.map(v => v.name).join(', ')}`)
                throw err
            }
        }
    }
